...
class BookingHandler:
    ...
    def handle_callback(self, user_id: int, payload: Dict[str, Any]) -> Tuple[str, Optional[Any]]:
        ...
        if action == 'back_to_dates':
            master_id = booking_service.get_temp_data(user_id, 'master_id')
            if master_id:
                booking_service.set_user_state(user_id, booking_service.STATE_SELECT_DATE)
        ...
        if action == 'select_service':
            ...
            service = booking_service.get_service_by_id(service_id)
            if service:
                booking_service.set_temp_data(user_id, 'service_id', service_id)
                booking_service.set_user_state(user_id, booking_service.STATE_SELECT_MASTER)
        ...
        if action == 'select_master':
            ...
            master = booking_service.get_master_by_id(master_id)
            if master:
                booking_service.set_temp_data(user_id, 'master_id', master_id)
                booking_service.set_user_state(user_id, booking_service.STATE_SELECT_DATE)
                return self._show_dates(user_id, master_id)
            else:
        ...
        if action == 'select_date':
            ...
            if master_id and date_str:
                booking_service.set_temp_data(user_id, 'selected_date', date_str)
                booking_service.set_user_state(user_id, booking_service.STATE_SELECT_TIME)
        ...
        if action == 'select_time':
            ...
            if slot_id:
                booking_service.set_temp_data(user_id, 'selected_slot_id', slot_id)
                booking_service.set_user_state(user_id, booking_service.STATE_CONFIRM)
        ...
        if action == 'confirm_booking':
            slot_id = payload.get('slot_id')
            if slot_id:
                result = booking_service.create_booking(user_id, slot_id)
                if result:
                    message = (f"✅ Вы успешно записаны!\n\n"
                               f"{booking_service.format_booking_message(result)}\n\n"
        ...
        if action == 'cancel_booking_by_id':
            ...
            if booking_id and booking_service.cancel_booking(user_id, booking_id):
...
💡 Выберите действие на клавиатуре ниже."""
    ...
    def _show_dates(self, user_id: int, master_id: int) -> Tuple[str, Optional[Any]]:
        ...
        dates = booking_service.get_available_dates(master_id, 7)
    ...
    def _show_time_slots(self, user_id: int, master_id: int, date_str: str) -> Tuple[str, Optional[Any]]:
        ...
        slots = booking_service.get_free_slots_by_date(master_id, date_str)
    ...
    def _show_confirmation(self, user_id: int, slot_id: int) -> Tuple[str, Optional[Any]]:
        ...
        service_id = booking_service.get_temp_data(user_id, 'service_id')
        master_id = booking_service.get_temp_data(user_id, 'master_id')
        
        service = booking_service.get_service_by_id(service_id) if service_id else None
        master = booking_service.get_master_by_id(master_id) if master_id else None
    ...
    def _show_user_bookings(self, user_id: int) -> Tuple[str, Optional[Any]]:
        ...
        for i, booking in enumerate(bookings, 1):
            message += f"{i}. {booking_service.format_booking_message(booking)}\n\n"
